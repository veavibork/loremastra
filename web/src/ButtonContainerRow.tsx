...
export default function ButtonContainerRow({
  ...
  function toggleCollapsed(containerId: string) {
    setCollapsed((prev) => {
      ...
      try {
        localStorage.setItem(`${COLLAPSE_STORAGE_PREFIX}${storageScope}.${containerId}`, String(nextVal));
      } catch {
        // ignore
      }
                ...
                {visibleButtons.map((btn) => {
                  ...
                  return (
                    <button
                      key={btn.id}
                      type="button"
                      className={[props.active ? "active" : "", props.className ?? ""].filter(Boolean).join(" ") || undefined}
