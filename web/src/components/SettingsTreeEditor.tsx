...
function buildCombinedTree(
  ...
  for (const s of sections) {
    ...
    if (s.parent) {
      const parentObj =
        typeof result[s.parent] === 'object' &&
        result[s.parent] !== null &&
        !Array.isArray(result[s.parent])
...
export default function SettingsTreeEditor({ sections }: { sections: SettingsSection[] }) {
  ...
  const [collapseSignal, setCollapseSignal] = useState<
    { path: string[]; collapsed: boolean; includeChildren: boolean } | undefined
  >()
  ...
  function exitRawMode() {
    ...
    if (missing.length > 0) {
      setError(
        `Missing section(s): ${missing.map((s) => (s.parent ? `${s.parent}.${sectionTreeKey(s)}` : s.title)).join(', ')}`,
      )
      return
    }
    setState((prev) => {
      ...
      for (const s of sections) {
        next[s.key] = {
          lastSaved: prev[s.key]?.lastSaved ?? s.value,
          draft: readSectionValue(obj, s)!,
        }
      }
      return next
    })
  ...
  async function handleSave() {
    ...
    if (rawMode) {
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? `Not saved — invalid JSON: ${err.message}`
            : 'Not saved — invalid JSON',
        )
        return
      }
      if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
      ...
      if (missing.length > 0) {
        setError(
          `Not saved — missing section(s): ${missing.map((s) => (s.parent ? `${s.parent}.${sectionTreeKey(s)}` : s.title)).join(', ')}`,
        )
        return
      }
      currentDraftByKey = Object.fromEntries(
        sections.map((s) => [s.key, readSectionValue(obj, s)!]),
      )
    } else {
      currentDraftByKey = Object.fromEntries(
        sections.map((s) => [s.key, state[s.key]?.draft ?? s.value]),
      )
    }

    const toSave = sections.filter(
      (s) => stringify(currentDraftByKey[s.key]) !== stringify(state[s.key]?.lastSaved),
    )
    try {
      ...
      if (rawMode) {
        setState((prev) => {
          const next = { ...prev }
          for (const s of sections) {
            if (!toSave.includes(s))
              next[s.key] = {
                lastSaved: prev[s.key]?.lastSaved ?? s.value,
                draft: currentDraftByKey[s.key],
              }
          }
          return next
        })
  ...
  return (
    <section className="settings-tree">
      ...
      <ul className="settings-tree-legend">
        {sections.map((s) => (
          <li key={s.key}>
            <strong>{s.parent ? `${s.parent} → ${sectionTreeKey(s)}` : s.title}</strong>
