...
export default function ArchivesView({ story }: PanelProps) {
  ...
  const { data: page } = useStoryToDate(storyId)
  const backfillMutation = useBackfillStoryToDateNames()
  const enqueueMutation = useEnqueueStoryToDate()
  ...
  useEffect(() => {
    if (storyId && !backfillTried.current && !page) {
      ...
      void backfillMutation.mutateAsync(storyId).catch(() => {})
    }
  }, [storyId, page, backfillMutation])
  ...
  const stats = {
    ...
    total: page?.total ?? 0,
    withContent: page?.withContent ?? 0,
    pending: page?.pending ?? 0,
    broken: page?.broken ?? 0,
  }
  ...
  async function saveEdit(segmentId: string) {
    ...
    if (
      coverageThroughIcPost !== undefined &&
      (!Number.isFinite(coverageThroughIcPost) || coverageThroughIcPost <= 0)
    ) {
  ...
  async function cancelMemoryJob(jobId: string) {
    if (!story) return
    setBusyKey(jobId)
    setError(null)
    try {
      await cancelJobMutation.mutateAsync({ storyId: story.id, jobId })
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err))
    } finally {
      setBusyKey(null)
    ...
    <div className="archives-view">
      <div className="archives-header">
        ...
        <div className="archives-header-actions">
          <button
            type="button"
            disabled={!!busyKey}
            onClick={() => void runAction('enqueue', () => enqueueMutation.mutateAsync(story.id))}
          >
            Queue job
          </button>
          ...
          {activeMemoryJobs.map((job) => (
            <div key={job.id} className="archives-active-job">
              ...
              <button
                ...
                onClick={() => void cancelMemoryJob(job.id)}
              >
                Cancel
              </button>
            </div>
      ...
      {stats.total === 0 && (
        <p className="archives-empty">
          No [STORY TO DATE] segments yet — queue a job or keep playing.
        </p>
      )}
      {stats.total > 0 && (
        <p className="archives-count">
          ...
          {stats.mergedCoverageThroughPost != null &&
            ` · archive through post ${stats.mergedCoverageThroughPost}`}
            ...
            >
                ...
                >
                  ...
                  <span className="archive-meta">
                    ...
                    {seg.coverageThroughIcPost != null &&
                      ` · through post ${seg.coverageThroughIcPost}`}
              ...
              <div className="archive-actions">
                {showRequeue && (
                  <button
                    ...
                    onClick={() =>
                      void runAction(seg.id, () =>
                        requeueMutation.mutateAsync({ storyId: story.id, segmentId: seg.id }),
                      )
                    }
                  >
                    Requeue
              ...
              {confirmDeleteId === seg.id && (
                <div className="archive-delete-confirm">
                  <span>
                    Remove this archive segment? Verbose posts it covered return to the Author
                    prompt.
                  </span>
                  <button
                    type="button"
