...
function readFileBounded(filePath: string, maxChars: number): { content: string; truncated: boolean } {
...
function grepCodebase(
...
): { matches: Array<{ file: string; line: number; text: string; context: string[] }>; filesSearched: number; truncated: boolean } {
  ...
  const skipDirs = new Set([
    "node_modules", "dist", ".git", "data", ".claude",
  ]);
...
server.registerTool(
  "ask_worker",
  {
    ...
    inputSchema: {
      ...
      files: z.array(z.string()).optional().describe("File paths (relative to cwd or absolute) to include as context for the question."),
      searchPattern: z.string().optional().describe("Regex pattern to grep across the codebase. Results are included as context before the question."),
      searchPath: z.string().optional().describe("Directory to search (relative to cwd or absolute). Defaults to cwd."),
      filePattern: z.string().optional().describe("Optional glob/regex to filter which files to search (e.g. '\\\\.ts$')."),
    },
  },
  async ({ question, files, searchPattern, searchPath, filePattern }) => {
    if (!API_KEY) {
      return textResult({
        error: "CLINE_WORKER_API_KEY is not set in .env. Add a Featherless API key to use the worker.",
      });
    }
    ...
    if (contextParts.length === 0) {
      return textResult({
        error: "No context provided. Include at least one of 'files' or 'searchPattern' so the worker has code to answer about.",
      });
    }
