...
export function getSettingsSpace<T>(db: Database.Database, userId: string, space: string, defaultValue: T): T {
  const row = db
    .prepare(`SELECT json_blob, previous_json_blob FROM settings_spaces WHERE space = ? AND user_id = ?`)
...
export function saveSettingsSpace<T>(db: Database.Database, userId: string, space: string, value: T): T {
  const existing = db
...
export function revertSettingsSpace<T>(db: Database.Database, userId: string, space: string): T | null {
  const row = db
    .prepare(`SELECT json_blob, previous_json_blob FROM settings_spaces WHERE space = ? AND user_id = ?`)
    .get(space, userId) as SettingsSpaceRow | undefined;
