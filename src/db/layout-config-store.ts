  ...
  const run = db.transaction(() => {
    ...
    db.prepare(
      `INSERT INTO layout_configs (id, user_id, name, config_json, is_active, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).run(id, input.userId, input.name, input.configJson, input.isActive ? 1 : 0, createdAt, createdAt);
  });
  run();
...
export function getLayoutConfig(db: Database.Database, id: string): LayoutConfigRow | null {
  const row = db.prepare(`SELECT * FROM layout_configs WHERE id = ?`).get(id) as RawLayoutConfigRow | undefined;
  return row ? mapRow(row) : null;
}

export function getActiveLayoutConfig(db: Database.Database, userId: string): LayoutConfigRow | null {
  const row = db
...
export function updateLayoutConfigJson(db: Database.Database, id: string, configJson: string): LayoutConfigRow {
  db.prepare(`UPDATE layout_configs SET config_json = ?, updated_at = ? WHERE id = ?`).run(configJson, nowIso(), id);
  return getLayoutConfig(db, id)!;
}
...
export function setActiveLayoutConfig(db: Database.Database, userId: string, id: string): void {
  const run = db.transaction(() => {
    ...
    db.prepare(`UPDATE layout_configs SET is_active = 1, updated_at = ? WHERE id = ? AND user_id = ?`).run(
      nowIso(),
      id,
      userId
    );
