...
export function fillArchiveName(db: Database.Database, id: string, name: string): boolean {
  const result = db
    .prepare(`UPDATE archive SET name = ? WHERE id = ? AND (name IS NULL OR name = '')`)
    .run(name, id)
  return result.changes > 0
}
...
export function syncArchiveMembersForPage(
  db: Database.Database,
  pageId: string,
  textId: string,
): void {
...
export function addArchiveMember(
  db: Database.Database,
  archiveId: string,
  textId: string,
  isOwner: boolean,
...
export function setArchiveMemberOwner(
  db: Database.Database,
  archiveId: string,
  textId: string,
  isOwner: boolean,
...
export function listMemberTextIds(db: Database.Database, archiveId: string): string[] {
  const rows = db
    .prepare(`SELECT text_id FROM archive_member WHERE archive_id = ?`)
