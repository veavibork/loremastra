...
export function getStoryToDateSegment(
  db: Database.Database,
  id: string,
): StoryToDateSegmentRow | null {
  const row = db.prepare(`SELECT * FROM story_to_date_segment WHERE id = ?`).get(id) as
    RawSegmentRow | undefined
...
export function fillStoryToDateSegmentName(
  db: Database.Database,
  id: string,
  name: string,
): boolean {
  const result = db
    .prepare(
      `UPDATE story_to_date_segment SET name = ? WHERE id = ? AND (name IS NULL OR name = '')`,
    )
    .run(name.trim(), id)
...
export function setStoryToDateSegmentContent(
  db: Database.Database,
  id: string,
  content: string,
): void {
...
export function deleteStoryToDateSegmentsFromSeq(
  db: Database.Database,
  bookId: string,
  fromSeq: number,
): void {
  ...
  db.prepare(`DELETE FROM story_to_date_segment WHERE book_id = ? AND seq >= ?`).run(
    bookId,
    fromSeq,
  )
}
