...
export function listStories(db: Database.Database, ownerUserId: string): StoryRow[] {
  const rows = db
    .prepare(
      `SELECT * FROM stories WHERE owner_user_id = ? AND hidden = 0 ORDER BY updated_at DESC`,
    )
...
export function listAllStories(db: Database.Database): StoryRow[] {
  const rows = db
    .prepare(`SELECT * FROM stories WHERE hidden = 0 ORDER BY updated_at DESC`)
    .all() as RawStoryRow[]
  return rows.map(mapStoryRow)
}
...
export function setStoryHidden(db: Database.Database, id: string, hidden: boolean): void {
  db.prepare(`UPDATE stories SET hidden = ?, updated_at = ? WHERE id = ?`).run(
    hidden ? 1 : 0,
    nowIso(),
    id,
  )
