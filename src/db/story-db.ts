...
function finishJobTypeCheckMigration(db: Database.Database): void {
  const exists = db
    .prepare(
      `SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'jobs_pre_tag_gen_migration'`,
    )
    .get()
  if (!exists) return
  db.exec(`
...
function finishJobTypeWorldbookCompactMigration(db: Database.Database): void {
  const exists = db
    .prepare(
      `SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'jobs_pre_worldbook_compact_migration'`,
    )
    .get()
  if (!exists) return
  db.exec(`
