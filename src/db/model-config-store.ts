...
export function createModelConfig(db: Database.Database, userId: string, input: ModelConfigInput): ModelConfigRow {
  const id = newId();
  const now = nowIso();
  const maxOrder = (db.prepare(`SELECT MAX(sort_order) AS m FROM model_configs WHERE user_id = ?`).get(userId) as {
    m: number | null;
  }).m;
...
export function updateModelConfig(db: Database.Database, id: string, patch: Partial<ModelConfigInput>): ModelConfigRow | null {
  ...
  const next: ModelConfigInput = {
    ...
    presencePenalty: patch.presencePenalty !== undefined ? patch.presencePenalty : current.presencePenalty,
    frequencyPenalty: patch.frequencyPenalty !== undefined ? patch.frequencyPenalty : current.frequencyPenalty,
    repetitionPenalty: patch.repetitionPenalty !== undefined ? patch.repetitionPenalty : current.repetitionPenalty,
...
export function reorderModelConfigs(db: Database.Database, userId: string, orderedIds: string[]): void {
  const update = db.prepare(`UPDATE model_configs SET sort_order = ?, updated_at = ? WHERE id = ? AND user_id = ?`);
  const now = nowIso();
...
export function recordModelOutcome(
  ...
  db.prepare(
    `UPDATE model_configs SET
     ...
     WHERE id = ?`
  ).run(outcome.success ? 1 : 0, outcome.success ? 0 : 1, outcome.inputTokens, outcome.outputTokens, id);
}
