...
export function normalizeWorldbookStoredContent(
  content: string,
  entryType: WorldbookEntryType,
): string {
...
function toEntry(
  row: RawWorldbookEntryRow,
  page: { bookId: string; hidden: boolean; broken: boolean; createdAt: string },
  text: TextRow,
): WorldbookEntry {
  return {
    pageId: row.page_id,
...
export function createWorldbookEntry(
  ...
  const run = db.transaction(() => {
    ...
    db.prepare(`INSERT INTO worldbook_entry (page_id, entry_type) VALUES (?, ?)`).run(
      page.id,
      input.entryType,
    )
    return { page, text }
...
export function getWorldbookEntry(db: Database.Database, pageId: string): WorldbookEntry | null {
  const row = db.prepare(`SELECT * FROM worldbook_entry WHERE page_id = ?`).get(pageId) as
    RawWorldbookEntryRow | undefined
  if (!row) return null
...
export function setWorldbookEntryHidden(
  db: Database.Database,
  pageId: string,
  hidden: boolean,
): void {
