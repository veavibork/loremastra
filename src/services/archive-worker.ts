...
export function buildArchiveProseBlob(
  ...
  for (const id of memberTextIds) {
    ...
    if (prose.length > ARCHIVE_MAX_PROSE_CHARS)
      prose = `${prose.slice(0, ARCHIVE_MAX_PROSE_CHARS)}…`
...
export function buildPriorArchiveContext(
  db: Database.Database,
  archive: ArchiveRow,
): string | null {
...
export function buildArchiveUserPrompt(db: Database.Database, targetArchiveId: string): string {
  ...
  parts.push(
    `Messages to archive (full prose only — summarize from these, do not invent events):\n${JSON.stringify(blob)}`,
  )
  return parts.join('\n\n')
}
