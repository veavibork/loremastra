...
export function formatCorpusForEditor(
  ...
  parts.push(
    '=== LOG (in-character verbose prose; post numbers are absolute from kickoff — hidden OOC turns occupy numbers but are omitted) ===',
  )
  for (const post of posts) {
...
const LEAKED_STORY_MARKER = /(?:\[|<)\/?STORY\s+(?:BEGINS|CONTINUES|TO\s+DATE|ENDS)(?:\]|>)/gi
...
export function buildDefaultContinuesSystemPrompt(
  ...
  const ceiling = buildSeamCeilingInstruction(inputCeilingPost)
  const coveredPosts =
    inputCeilingPost != null && priorCoveragePost != null
      ? inputCeilingPost - priorCoveragePost
      : null
...
export function shouldRetrySeamGate(
  coverageThroughPost: number,
  inputCeilingPost: number | null,
): boolean {
