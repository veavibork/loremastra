...
export function tryShortVerbatimCompress(
  role: TextRole,
  content: string,
): { summary: string } | null {
...
export function validateCompressSummary(
...
export function sanitizeCompressResult(
  messageContent: string,
...
function collectPriorProseSnippets(
  db: Database.Database,
  targetPage: PageRow,
): Array<{ role: TextRole; snippet: string }> {
  ...
  while (currentId && snippets.length < PRIOR_PROSE_MESSAGES) {
    ...
    if (text?.genPackage?.trim()) {
      ...
      const truncated =
        plain.length > PRIOR_PROSE_MAX_CHARS ? `${plain.slice(0, PRIOR_PROSE_MAX_CHARS)}…` : plain
...
export function buildCompressUserPrompt(
  db: Database.Database,
  targetText: TextRow,
  targetPage: PageRow,
): string {
