...
export function enqueuePendingArchiveNameJobs(db: Database.Database, userId: string, logbookId: string): number {
  let enqueued = 0;
  for (const archive of listArchivesForBook(db, logbookId)) {
    if (!archive.summary?.trim() || archive.broken) continue;
...
export function requeueArchiveBlock(db: Database.Database, userId: string, archiveId: string): void {
