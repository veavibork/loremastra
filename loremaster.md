...
- **Editor input:** worldbook + verbose prose, plus the prior merged `[STORY TO DATE]` for `[STORY CONTINUES]`. `[STORY BEGINS]` takes the log up to the **80%** token cutoff; `[STORY CONTINUES]` is **windowed to the next 12 visible posts after prior coverage** (`NEXT_SCENE_INPUT_WINDOW_POSTS`; 24 at first, halved 2026-07-17 after the verify-ab experiment showed the model runs coverage to the input ceiling on essentially every call — the window, not the prompt, is the effective scene budget) — the input ceiling structurally caps how far `[COVERAGE]` can claim, so a segment can never skip past unsummarized scenes (2026-07-17 fix; a 60-post backlog once produced a segment that claimed 56 posts while summarizing only the final scene). Catch-up over a backlog happens by chaining jobs — the executor re-checks the trigger after each fill.
