...
import { createJob, getJob, claimNextJob, finishJob, hasActiveJobForText, listRecentJobs, listActiveJobs, listPendingJobs } from "../../src/db/job-store.js";
...
describe("createJob", () => {
  ...
  it("throws when multiple targets specified", () => {
    expect(() => createJob(db, { targetTextId: "t3", targetArchiveId: "a1", jobType: "prose" }))
      .toThrow("exactly one");
  });
...
describe("finishJob", () => {
  ...
  it("records metadata", () => {
    ...
    finishJob(db, job.id, "done", undefined, { model: "test-model", tokenEstimate: 42, elapsedMs: 150 });
    const updated = getJob(db, job.id)!;
