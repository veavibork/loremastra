...
import { createText, getText, fillTextGeneration, clearTextExtract, setTextHidden, listSelectedTextsForBook } from "../../src/db/text-store.js";
...
describe("createText", () => {
  ...
  it("sets priorTextId and genRequest", () => {
    const text = createText(db, { pageId: "p2", role: "agent", priorTextId: "t0", genRequest: "request body" });
...
describe("fillTextGeneration", () => {
  it("fills gen_package and gen_metrics write-once", () => {
    ...
    expect(fillTextGeneration(db, text.id, { genPackage: "reply", genMetrics: JSON.stringify({ elapsedMs: 100 }) })).toBe(true);
...
describe("clearTextExtract", () => {
  it("clears gen_extract and compress_metrics", () => {
    ...
    db.prepare("UPDATE text SET gen_extract = ?, compress_metrics = ? WHERE id = ?").run("old extract", "old metrics", text.id);
...
describe("listSelectedTextsForBook", () => {
  it("returns texts for selected pages in a book", () => {
    ...
    db.prepare("INSERT INTO page (id, created_at, book_id, prev_page_id, selected_fork_page_id, selected_text_id, select_time, hidden, broken) VALUES ('pg1', datetime('now'), ?, NULL, NULL, NULL, NULL, 0, 0)",).run(bookId);
