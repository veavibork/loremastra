...
console.log({ inputCeilingPost: corpus.inputCeilingPost, included: corpus.includedPosts.length, posts: corpus.posts.length });
console.log(`post ${n} in included:`, corpus.includedPosts.some((p) => p.icPostNumber === n));
console.log(`post ${n} chain:`, entry ? { hidden: entry.hidden } : null);
console.log(
  "included nearby:",
  corpus.includedPosts.filter((p) => p.icPostNumber >= n - 3 && p.icPostNumber <= n + 3).map((p) => p.icPostNumber)
);
