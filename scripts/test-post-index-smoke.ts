...
import { buildStoryCorpus, formatCorpusForEditor } from "../src/services/story-to-date-corpus.js";
...
function main(): void {
  ...
  for (const entry of logEntries) {
    ...
    assert(entry.icPostNumber === expected, `Logs post # matches chain index for ${entry.pageId.slice(0, 8)}`);
  }
  ...
  const previewPosts = preview.messages.filter((m) => m.icPostNumber != null).map((m) => m.icPostNumber!);
