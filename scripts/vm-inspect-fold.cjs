...
for (const file of fs
  .readdirSync(dir)
  .filter((f) => f.endsWith('.sqlite') && !f.includes('smoke'))) {
  ...
  for (const s of segs) {
    if (
      ...
      String(s.coverage_page_id || '')
        .toLowerCase()
        .startsWith(prefix)
    ) {
      const content =
        db.prepare('SELECT content FROM story_to_date_segment WHERE id = ?').get(s.id)?.content ||
        ''
      console.log('---')
      console.log('story:', file.replace('.sqlite', ''))
      console.log('segment:', s.id)
      console.log(
        `seq ${s.seq} ${s.kind} cov ${s.coverage_through_ic_post} page ${s.coverage_page_id}`,
      )
  ...
  if (segs.length) {
    ...
    for (const s of segs) {
      ...
      const gap =
        prev && prev.cov != null && s.coverage_through_ic_post != null
          ? s.coverage_through_ic_post - prev.cov
          : null
      const _covGap =
        prev && prev.cov != null && s.coverage_through_ic_post != null && gap > 1
          ? ` POST-GAP +${gap}`
          : ''
      if (
        prev &&
        prev.cov != null &&
        s.coverage_through_ic_post != null &&
        s.coverage_through_ic_post <= prev.cov
      ) {
        console.log(
          `[WARN ${file}] seq ${s.seq} cov ${s.coverage_through_ic_post} does not advance past ${prev.cov}`,
        )
      }
      prev = { seq: s.seq, cov: s.coverage_through_ic_post, tok }
    }
    const _total = segs.reduce((sum, s) => {
      const row = db.prepare('SELECT content FROM story_to_date_segment WHERE id = ?').get(s.id)
    ...
    if (filled.length >= 2) {
      ...
      if (foldEnd && keepStart && foldEnd.cov != null && keepStart.cov != null) {
        ...
        if (between > 5) {
          console.log(
            `[FOLD-GAP ${file}] fold ends cov ${foldEnd.cov} (seq ${foldEnd.seq}), keep starts seq ${keepStart.seq} cov ${keepStart.cov} (+${between} posts)`,
          )
        }
      }
    }
