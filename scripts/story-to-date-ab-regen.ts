...
async function runVariant(
  ...
  for (let seq = 0; seq < maxBlocks; seq++) {
    ...
    if (kind === 'continues' && corpus.posts.length <= MIN_VERBOSE_IC_POSTS) {
      log(
        `only ${corpus.posts.length} posts left after coverage ${priorCoverage} (≤ verbose tail ${MIN_VERBOSE_IC_POSTS}) — stopping; these stay verbatim in production.`,
      )
      reachedHead = true
      break
    }
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS && !committed; attempt++) {
      ...
      if (corpus.inputCeilingPost != null && coverage > corpus.inputCeilingPost) {
        log(
          `seq ${seq} attempt ${attempt}: coverage ${coverage} > ceiling ${corpus.inputCeilingPost} — retrying`,
        )
        continue
      }
      ...
      if (!coveragePost) {
        log(
          `seq ${seq} attempt ${attempt}: coverage ${coverage} not in included input (ceiling ${corpus.inputCeilingPost}) — retrying`,
        )
        continue
      }
      if (priorCoverage != null && coverage <= priorCoverage) {
        log(
          `seq ${seq} attempt ${attempt}: coverage ${coverage} does not advance past ${priorCoverage} — retrying`,
        )
        continue
      }
...
async function main(): Promise<void> {
  if (VARIANT_B_GUIDANCE === INCLUDE_EXCLUDE_GUIDANCE) {
    throw new Error(
      "Variant B guidance is identical to baseline — the .replace() didn't match. Fix before running.",
    )
  }
  ...
  if (!apiKey)
    throw new Error(
      'no Featherless API key (set FEATHERLESS_API_KEY or provide APP_MASTER_KEY for DB key)',
    )
  ...
  for (const v of variants) {
    ...
    writeFileSync(
      join(outDir, 'system-prompt-sample-continues.txt'),
      buildDefaultContinuesSystemPrompt(500, 450, { guidance: v.guidance }),
    )
