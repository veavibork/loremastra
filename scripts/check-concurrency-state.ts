...
import { ensureConcurrencyFeedForUser, getConcurrencySnapshot, isFeedHealthy } from "../src/queue/concurrency-feed.js";
...
const slotCosts = db
  .prepare(`SELECT slot_cost, status, COUNT(*) AS n FROM jobs WHERE job_type = 'archive' GROUP BY slot_cost, status ORDER BY slot_cost, status`)
  .all();
