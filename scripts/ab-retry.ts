...
if (!apiKey) { console.error("set FEATHERLESS_API_KEY"); process.exit(1); }
...
const tools = [{ type: "function", function: { name: "write_file", description: "Write content to a file", parameters: { type: "object", properties: { path: { type: "string" }, content: { type: "string" } }, required: ["path", "content"] } } }];
  ...
  for (const model of MODELS) {
    ...
    try {
      const r = await fetch(`${FEATHERLESS_BASE_URL}/chat/completions`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "User-Agent": FEATHERLESS_USER_AGENT, Authorization: `Bearer ${apiKey}` },
        body: JSON.stringify({ model, messages: [{ role: "system", content: systemPrompt }, { role: "user", content: userPrompt }], temperature: 0, max_tokens: 2048, stream: false, tools, tool_choice: "auto" }),
      });
      const ms = Math.round(performance.now() - start);
      if (!r.ok) { const body = await r.text().catch(() => ""); console.log(`${model}: FAIL HTTP ${r.status} — ${body.slice(0, 200)}`); continue; }
      const d = await r.json() as { usage?: { prompt_tokens?: number; completion_tokens?: number }; choices?: Array<{ message?: { content?: string | null; tool_calls?: Array<{ function?: { name?: string; arguments?: string } }> } }> };
      ...
      if (tcName === "write_file") {
        try { const args = JSON.parse(tc?.arguments ?? "{}") as { path?: string; content?: string }; path = args.path ?? ""; content = args.content ?? ""; } catch { /* ignore */ }
      }
      const hasAll = content.includes("## How listModels") && content.includes("## How the eval script") && content.includes("## One improvement");
      const wc = content.split(/\s+/).filter(Boolean).length;
      const expectedPath = `test-results/${model.split("/").pop()}-analysis.md`;
      console.log(`${model}: ${ms}ms | ${tokIn}/${tokOut} tok | tool="${tcName}" | path="${path}" ${path === expectedPath ? "✓" : "✗"} | sections=${hasAll ? "✓" : "✗"} | words=${wc}`);
    } catch (e) {
