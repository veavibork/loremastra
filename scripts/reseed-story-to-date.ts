...
async function main() {
  ...
  const existing = listStoryToDateSegments(db, logbook.id, {
    includeHidden: true,
    includeBroken: true,
  })
  ...
  for (; seq < 100; seq++) {
    ...
    const priorRows = listStoryToDateSegments(db, logbook.id).filter(
    ...
    if (corpus.includedPosts.length === 0) {
      console.log(`Reached head at coverage ${priorCoverage}.`)
      break
    }
    if (kind === 'continues' && corpus.posts.length <= MIN_VERBOSE_IC_POSTS) {
      console.log(
        `Only ${corpus.posts.length} posts left after ${priorCoverage} — leaving verbose tail.`,
      )
      break
    }
    ...
    for (let attempt = 1; attempt <= 2 && !committed; attempt++) {
      ...
      if (
        ...
        const rb = extractStoryBlock(rr, kind)
        const rc = extractCoverage(rr)
      ...
      if (block && coverage != null && kind === 'continues' && priorCoverage != null) {
        ...
        if (looksNextSceneCoverageSprint(block, delta)) {
          const retry = [
            ...
            {
              role: 'user' as const,
              content: buildCoverageSprintRetryUserMessage(kind, coverage, priorCoverage),
            },
          ]
          const rr = await chat(editor, apiKey, retry)
          const rb = extractStoryBlock(rr, kind)
          ...
          if (
            ...
            rc != null &&
            rc < coverage &&
            !looksNextSceneCoverageSprint(rbSan, rc - priorCoverage)
          ) {
            block = rbSan
            coverage = rc
          }
      ...
      if (!block || coverage == null) {
        console.log(`  seq ${seq} attempt ${attempt}: no block/coverage`)
        continue
      }
      block = sanitizeStoryBlockContent(block)
      if (!block) {
        console.log(`  seq ${seq} attempt ${attempt}: empty after sanitization`)
        continue
      }
      if (kind === 'continues' && priorRows.length) {
        ...
        if (overlap >= STORY_BLOCK_DUPLICATE_OVERLAP_THRESHOLD) {
          console.log(
            `  seq ${seq} attempt ${attempt}: duplicate of prior (${(overlap * 100).toFixed(1)}% overlap)`,
          )
          continue
        }
      }
      if (corpus.inputCeilingPost != null && coverage > corpus.inputCeilingPost) {
        console.log(`  seq ${seq}: coverage>${corpus.inputCeilingPost}`)
        continue
      }
      const chainEntry = buildChainPostIndex(db, logbook.id).find((e) => e.postNumber === coverage)
      if (!chainEntry || chainEntry.hidden) {
        console.log(`  seq ${seq}: coverage ${coverage} off visible chain`)
        continue
      }
      const cp = corpus.includedPosts.find((p) => p.icPostNumber === coverage)
      if (!cp) {
        console.log(`  seq ${seq}: coverage ${coverage} not in input`)
        continue
      }
      if (priorCoverage != null && coverage <= priorCoverage) {
        console.log(`  seq ${seq}: no advance past ${priorCoverage}`)
        continue
      }
      if (kind === 'continues' && priorCoverage != null) {
        ...
        if (looksNextSceneCoverageSprint(block, delta)) {
          console.log(
            `  seq ${seq} attempt ${attempt}: coverage sprint +${delta} in ${storyBlockWordCount(block)}w`,
          )
          continue
        }
      }
      ...
      afterPageId = cp.pageId
      priorCoverage = coverage
      committed = true
      console.log(
        `  seq ${seq} ${kind}: cov →${coverage}/${headPost} (${estimateTokens(block)} tok)`,
      )
    }
    if (!committed) {
      console.log(`  seq ${seq}: FAILED — stopping regeneration.`)
      break
    }
  }
  ...
  const final = listStoryToDateSegments(db, logbook.id).filter(
    (s) => s.content?.trim() && !s.broken,
  )
