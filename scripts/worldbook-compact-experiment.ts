...
async function completeChatRetrying(
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      console.log(`  (429 concurrency — waiting ${wait / 1000}s for a slot, retry ${attempt + 1}/${delays.length})`);
      await sleep(wait);
    }
  }
}
...
async function main(): Promise<void> {
  ...
  if (!storyId) {
    console.error("usage: worldbook-compact-experiment.ts <storyId> [--only TYPE] [--limit N] [--dry-run]");
    process.exit(1);
  }
  ...
  if (!apiKey && !dryRun) throw new Error("no Featherless API key (set FEATHERLESS_API_KEY or provide APP_MASTER_KEY)");
