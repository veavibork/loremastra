...
if (!storyId) {
  console.error(
    'Usage: npx tsx scripts/repair-story-post-index.ts <storyId> [--fix] [--delete-segments] [--requeue]',
  )
  process.exit(1)
}
...
if (deleteSegments) {
  ...
  for (const seg of segments) {
    ...
    if (
      fix &&
      seg.inputCeilingPageId &&
      fromCeilingPage != null &&
      fromCeilingPage !== storedCeiling
    ) {
      db.prepare(`UPDATE story_to_date_segment SET input_ceiling_ic_post = ? WHERE id = ?`).run(
        fromCeilingPage,
        seg.id,
      )
