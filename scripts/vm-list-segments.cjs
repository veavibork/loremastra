...
for (const s of segs) {
  const content =
    db.prepare('SELECT content FROM story_to_date_segment WHERE seq = ? AND broken = 0').get(s.seq)
      ?.content || ''
  const tok = estimateTokens(content)
  total += tok
  const delta =
    prevCov != null && s.coverage_through_ic_post != null
      ? s.coverage_through_ic_post - prevCov
      : null
...
const filled = segs
  .map((s) => {
    const content =
      db.prepare('SELECT content FROM story_to_date_segment WHERE seq = ?').get(s.seq)?.content ||
      ''
...
if (keepFrom > 0 && keepFrom < filled.length) {
  console.log(
    `  fold span ends cov ${filled[keepFrom - 1].cov}, keep starts cov ${filled[keepFrom].cov} (+${filled[keepFrom].cov - filled[keepFrom - 1].cov} posts)`,
  )
}
