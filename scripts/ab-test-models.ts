...
const MODELS = {
  ...
  '2-slot': ['Qwen/Qwen2.5-Coder-32B-Instruct', 'Qwen/Qwen2.5-32B-Instruct'],
}

// ── Prompt ────────────────────────────────────────────────────

const evalScript = readFileSync(
  join(import.meta.dirname ?? '.', 'evaluate-worker-models.ts'),
  'utf-8',
)
const feathModels = readFileSync(
  join(import.meta.dirname ?? '.', '..', 'src', 'inference', 'featherless-models.ts'),
  'utf-8',
)
...
async function main() {
  ...
  for (const [tier, models] of Object.entries(MODELS)) {
    for (const model of models) {
      ...
      try {
        ...
        console.error(
          `  ${status} | ${r.durationMs}ms | ${r.inputTokens}+${r.outputTokens} tok | ${s.wordCount}w`,
        )
      } catch (err) {
  ...
  for (const s of allScores) {
    ...
    console.log(
      `| ${short} | ${s.tier} | ${s.durationMs} | ${s.inputTokens} | ${s.outputTokens} | ${path} | ${secs} | ${s.wordCount} | ${errs} |`,
    )
  }
}
