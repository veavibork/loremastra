...
async function completeChatRetrying(editor: any, apiKey: string, messages: ChatMessage[]): Promise<string> {
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      console.log(`  (429 — waiting ${delays[attempt]! / 1000}s, retry ${attempt + 1}/${delays.length})`);
...
async function main(): Promise<void> {
  ...
  if (!storyId) {
    console.error("usage: story-to-date-fold-experiment.ts <storyId> [--keep-recent N] [--target-words N]");
    process.exit(1);
  }
  const keepRecent = args.indexOf("--keep-recent") >= 0 ? Number(args[args.indexOf("--keep-recent") + 1]) : 5;
  const targetArg = args.indexOf("--target-words") >= 0 ? Number(args[args.indexOf("--target-words") + 1]) : null;
  ...
  if (segs.length <= keepRecent) throw new Error(`only ${segs.length} segments; nothing to fold with keep-recent=${keepRecent}`);
  ...
  console.log(`Folding oldest ${oldSegs.length} segments (through post ${foldedCoverage}), keeping recent ${recentSegs.length}.`);
    ...
    JSON.stringify(
      {
        ...
        oldWords, oldTok, targetWords, digestWords, digestTok,
  ...
  console.log(`STORY TO DATE total: ${totalBefore} → ${totalAfter} tok  (${Math.round((1 - totalAfter / totalBefore) * 100)}% cut, and now BOUNDED)`);
