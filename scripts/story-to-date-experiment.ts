...
function checkCoverage(
  ...
  if (mode === "continues" && priorCoveragePost != null && coverageThroughPost <= priorCoveragePost) {
...
async function cmdRun(storyId: string, args: string[]): Promise<void> {
  ...
  let priorStoryToDate = argValue(args, "--prior") ? readFileSync(argValue(args, "--prior")!, "utf-8") : undefined;
  ...
  if (mode === "continues" && !priorStoryToDate?.trim()) {
    throw new Error("continues mode needs --prior <file> or --from-artifact <dir> with segments.json");
  }
  ...
  if (!parsed) {
    console.error("\nNo [STORY " + mode.toUpperCase() + "] block or [COVERAGE] in response. See response-raw.txt");
    process.exit(1);
  }
  ...
  if (
    !noSeamRetry &&
    ceiling != null &&
    shouldRetrySeamGate(parsed.coverageThroughPost, ceiling)
  ) {
    console.log(
  ...
  if (fromArtifact && existsSync(join(fromArtifact, "segments.json"))) {
    segments.push(...(JSON.parse(readFileSync(join(fromArtifact, "segments.json"), "utf-8")) as StoryToDateSegment[]));
  }
