...
const OUT_PATH = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../src/data/hf-model-tags.json");
...
async function main() {
  ...
  const out: Record<string, { tags: string[]; updatedAt: string }> = existsSync(OUT_PATH)
    ? (JSON.parse(readFileSync(OUT_PATH, "utf-8")) as Record<string, { tags: string[]; updatedAt: string }>)
    : {};
