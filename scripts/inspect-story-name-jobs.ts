...
for (const j of jobs) {
  console.log(`  ${j.status} ${j.created_at} text=${j.target_text_id?.slice(0, 8) ?? "—"} ${j.error ?? ""}`);
}
...
if (icStartPageId) {
  ...
  if (kickoffPage) {
    const textIds = db
      ...
      .all(kickoffPage.id) as { id: string }[];
    ...
    for (const { id } of textIds) {
      ...
      if (proseJobs.length) {
        console.log(`  text ${id.slice(0, 8)}: ${proseJobs.map((j) => `${j.job_type}/${j.status}`).join(", ")}`);
      }
    }
  }
}
