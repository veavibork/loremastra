...
async function main(): Promise<void> {
  ...
  assert(
    created.status === 200,
    `POST /api/stories creates story (got ${created.status}: ${JSON.stringify(created.body)})`,
  )
  ...
  try {
    ...
    assert(
      (
        await api(base, `/api/stories/${storyId}/worldbook`, {
          ...
          body: JSON.stringify({
            entryType: 'roster',
            content: 'Dragon — ancient wyrm guarding the pass.',
          }),
        })
      ).status === 200,
      ...
      assert(
        (
          await api(base, `/api/stories/${storyId}/messages`, {
            method: 'POST',
            body: JSON.stringify({
              content: `The party asks the Dragon for passage (turn ${pair}).`,
            }),
          })
        ).status === 200,
    ...
    assert(
      JSON.stringify(preview.body).includes('Dragon'),
