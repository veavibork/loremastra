import {
  listModels,
  suggestFiltersForRole,
  type FeatherlessModel,
} from '../src/inference/featherless-models.js'
...
const MATH_SPECIFIC_PATTERNS: RegExp[] = [/-Math-/i, /-Math$/i]
...
const LOW_REPUTATION_ORGS: Record<string, true> = {
  ...
  Luni: true,
  'Delta-Vector': true,
  SicariusSicariiStuff: true,
  UsernameJustAnother: true,
  rubenroy: true,
  limloop: true,
  Umranz: true,
  'allura-org': true,
...
function analyzeModel(m: FeatherlessModel) {
  ...
  else if (paramsB >= 3 && paramsB <= 14) score += 1
...
async function main() {
  ...
  if (baseline) {
    ...
    for (const c of above.slice(0, 15)) {
      console.log(
        `    - \`${c.id}\` (score ${c.score}, ${c.family} ${c.paramsB}B ${c.isInstruct ? 'INSTRUCT' : ''})`,
      )
    }
  } else {
