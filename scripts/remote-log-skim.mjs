...
const dbFiles = existsSync(storyDir) ? readdirSync(storyDir).filter((f) => f.endsWith(".sqlite")) : [];
...
console.log(`scanned ${totalScanned} prose/setup replies across ${dbFiles.length} stories, ${totalFlagged} flagged\n`);

for (const { file, flagged } of flaggedByStory) {
  console.log(`--- ${file} (${flagged.length} flagged) ---`);
  for (const r of flagged) {
    console.log(`job ${r.job_id} | ${r.created} | ${r.job_type} | ${r.model ?? "—"} | flags: ${r.flags.join(", ")}`);
