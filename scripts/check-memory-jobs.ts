...
const archives = db.prepare(`SELECT COUNT(*) AS total, SUM(CASE WHEN summary IS NOT NULL THEN 1 ELSE 0 END) AS filled FROM archive`).get() as {
  total: number;
  filled: number;
};
console.log(JSON.stringify({ storyId, jobs: rows, extractsWithSummary: extracts.n, archives }, null, 2));
