...
function inspect(storyId: string) {
  ...
  const total = db
    .prepare(`SELECT status, COUNT(*) AS n FROM jobs WHERE job_type = 'archive' GROUP BY status ORDER BY status`)
    .all();
  const failed = db
    .prepare(
  ...
  const pendingJobs = db
    .prepare(`SELECT COUNT(*) AS n FROM jobs WHERE job_type = 'archive' AND status IN ('pending','running')`)
    .get() as { n: number };
  return { storyId, total, failed, recent, pendingArchives: pendingArchives.n, pendingArchiveJobs: pendingJobs.n, models };
}
...
if (storyFilter) {
  ...
  for (const f of readdirSync("data/stories").filter((x) => x.endsWith(".sqlite"))) {
    ...
    const hasFailed = (result.total as Array<{ status: string; n: number }>).some((r) => r.status === "failed");
