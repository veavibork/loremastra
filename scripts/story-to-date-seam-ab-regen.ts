...
function loadProductionBeginsSeed(db: any, logbookId: string): StoryToDateSegment | null {
  const row = db
    .prepare(
...
async function runVariant(
  ...
  for (let seq = 1; seq < maxBlocks; seq++) {
    ...
    if (corpus.posts.length <= MIN_VERBOSE_IC_POSTS) {
      log(`only ${corpus.posts.length} posts remain (≤ verbose tail ${MIN_VERBOSE_IC_POSTS}) — stopping.`);
      return { metrics, segments, reachedHead: true, headPost };
    }
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS && !committed; attempt++) {
      ...
      if (cfg.rejectDuplicates && overlap >= 0.85) {
        ...
        log(`seq ${seq} attempt ${attempt}: duplicate of prior (${(overlap * 100).toFixed(1)}% overlap) — retrying`);
        continue;
      }
...
function summarizeVariant(metrics: BlockMetric[], headPost: number, reachedHead: boolean) {
  ...
  const avgDelta = continues.length
    ...
    : 0;
  const avgWords = continues.length ? continues.reduce((a, m) => a + m.words, 0) / continues.length : 0;
...
async function main(): Promise<void> {
  ...
  const root = join("data", "experiments", "story-to-date", `seam-ab-${stamp}-${storyId.slice(0, 8)}`);
